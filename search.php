...
<?php 
    ...
    if ($var == "search") {
        ...
        // Verificamos na base de dados produtos equivalente a palavra digitada
       if($searchText != '' && $searchDistrito != 0){
		...
		$numRegistros = mysql_num_rows($sqlText);
		 ...
		 $sqlText = mysql_query("SELECT * FROM empresas WHERE id_distrito=$searchDistrito ORDER BY id_empresa" );
		$numRegistros = mysql_num_rows($sqlText);  
	   }
    ...
    <div class="row">
      ...
      <div id="container" class="col-xs-12 col-sm-12 col-md-9">
        ...
        if (@$numRegistros != 0) {
            ...
            // Exibe os produtos e os seus respectivos preços
        ...
        ?>
            <div class="content_search" style="border-bottom:1px solid #E0E0E0;">
              <div class="col-sm-6 col-md-4">
                <div class="thumbnail"> <img src="<?php echo 'autenticacao/empresas/'.$results['logo']; ?>" alt="..."> </div>
              </div>
            ...
            // Se não houver registos
            }
        }
        else{?>
			<div class="alert alert-info" role="alert"><?php echo 'Nenhuma empresa foi encontrada<strong>'.$searchText.'</strong>'; ?></div>
        <?php }?>
     </div>
	</div>
    </div>
          ...
          <div class="panel-body">
			...
			$num_row = mysql_num_rows($result2) or die (mysql_error());
                ...
                while ($row2 = mysql_fetch_array($result2))
                    ...
                    echo "Nenhuma empresa foi encontrada";
            ?>
          </div>
        </div>
...
<?php
	...
	/*?>/*<!--else if ($id_atividade == $id_atividade) {
        
                
	   if($id_atividade != 0){
		 $sqlText = mysql_query("SELECT * FROM empresas WHERE id_atividade=$id_atividade ORDER BY id_empresa" );
		$numRegistros = mysql_num_rows($sqlText);  
		echo "SELECT * FROM empresas WHERE id_atividade=$id_atividade ORDER BY id_empresa";
	   }
